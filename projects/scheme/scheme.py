...
def eval_all(expressions, env):
    ...
    while expressions != nil:
        res = scheme_eval(expressions.first, env, tail=True if expressions.rest is nil else False)
...
def do_if_form(expressions, env):
    ...
    if is_true_primitive(scheme_eval(expressions.first, env)):
        return scheme_eval(expressions.rest.first, env, tail=True if expressions.rest is nil else False)
    elif len(expressions) == 3:
        return scheme_eval(expressions.rest.rest.first, env, tail=True if expressions.rest is nil else False)
