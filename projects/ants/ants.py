...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        current_range = 0
        while current_range < self.min_range:
          current_place = current_place.entrance
          if current_place is beehive:
            return None
          current_range += 1

        while current_place is not beehive and current_range <= self.max_range:
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        ...
        Ant.__init__(self, armor)
        self.is_queue = False
        if QueenAnt.has_queen is False:
            self.is_queue = True
    ...
    @staticmethod
    def buffer_ant(ant):
        if ant is not None and ant.buffered is False:
            ant.buffered = True
            ant.damage *= 2
    ...
    def action(self, gamestate):
        ...
        if self.is_queue:
            ...
            while place is not None:
                QueenAnt.buffer_ant(place.ant)
                if isinstance(place.ant, ContainerAnt):
                    QueenAnt.buffer_ant(place.ant.contained_ant)
                place = place.exit
        else:
...
def apply_effect(effect, bee, duration):
    ...
    # BEGIN Problem EC
    orignal_action = bee.action
    new_action = effect(orignal_action, bee)
    def action_add_effect(gamestate):
        nonlocal duration
        if duration > 0:
            new_action(gamestate)
            duration -= 1
        else:
            orignal_action(gamestate)
    bee.action = action_add_effect
    # END Problem EC
...
class LaserAnt(ThrowerAnt):
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem OPTIONAL
        damage = 2 - 0.2 * distance - self.insects_shot * 0.05
