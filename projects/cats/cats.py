...
def accuracy(typed, reference):
  ...
  for typed_word in typed_words:
    if counts < len(reference_words) and typed_word == reference_words[counts]:
      correct_words += 1
    total_words += 1
    counts += 1
  return 0.0 if total_words == 0 else correct_words / total_words * 100
  # END PROBLEM 3
...
def autocorrect(user_word, valid_words, diff_function, limit):
  ...
  return user_word if diff_function(user_word, valid_word, limit) > limit else valid_word
  # END PROBLEM 5
...
def feline_fixes(start, goal, limit):
  ...
  if not start or not goal or limit < 0:  # Fill in the condition
    # BEGIN
    return max(len(start), len(goal), int(limit < 0))
    # END
...
def report_progress(typed, prompt, id, send):
  ...
  total = len(prompt)
  count = 0
  for word_id, word in enumerate(typed):
    if word_id >= total or word != prompt[word_id]:
      break
    count += 1
...
def time_per_word(times_per_player, words):
  ...
  for times in times_per_player:
    game_times.append([times[i + 1] - times[i] for i in range(len(times) - 1)])
...
def fastest_words(game):
  ...
  # BEGIN PROBLEM 10
  fastest_words_list = [[] for _ in players]
  for word_index in words:
    times = [time(game, player, word_index) for player in players]
    fastest_player = times.index(min(times))
    fastest_words_list[fastest_player].append(word_at(game, word_index)) 
