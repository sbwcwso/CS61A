...
def accumulate(combiner, base, n, f):
    ...
    i = 1;
    while i <= n:
        base = combiner(f, base) if callable(base) else combiner(f(i), base)
        i += 1
    return base
...
def make_repeater(h, n):
    ...
    5
    """
    return accumulate(compose1, identity, n, h)
...
def add_church(m, n):
    ...
    #     n = successor(n)
    # return n
    return successor(n) if(church_to_int(m) == 1) else \
    accumulate(add_church, m, church_to_int(n), one)
...
def mul_church(m, n):
    ...
    12
    """
    return accumulate(add_church, zero, church_to_int(m), n)
...
def pow_church(m, n):
    ...
    9
    """
    return accumulate(mul_church, one, church_to_int(n), m)